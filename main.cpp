...
// Program to manage a car owner's registry using structures, pointers, and dynamic memory allocation
// This could be useful for a car dealership or rental service.
...
struct carRegistry {
    ...
    string licensePlate; // License plate number, makes sense for it to also be a dynamic array, but kept it simple
...
void inputRegistryData(carRegistry *regPtr) {
    static int ownerCount = 1; // static variable as index
    ...
    // Clear the input buffer after reading numofCar
    cin.ignore();
...
// Function to display data for a car registry entry
void displayRegistryData(carRegistry *regPtr) {
    static int i = 0; // Static variable to track display count
